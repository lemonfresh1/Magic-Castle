...
func export_to_png(custom_output_path: String = "") -> bool:
	var output_path = custom_output_path if custom_output_path != "" else "res://Pyramids/assets/icons/card_backs/%s.png" % item_id
	...
	canvas.draw.connect(func(): draw_card_back(canvas, canvas.size))
...
func create_item_data() -> UnifiedItemData:
	...
	item.id = item_id
	item.display_name = display_name if display_name != "" else theme_name + " Card Back"
